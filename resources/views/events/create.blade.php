...
<div class="max-w-3xl mx-auto" x-data="eventCreateForm()">
    <h1 class="text-2xl font-bold text-amber-400 mb-6">📅 新規イベント作成</h1>
    ...
    <form method="POST" action="{{ route('events.store') }}">
        ...
        <div class="card p-5 mb-4">
            ...
            <div class="mt-3">
                <label class="block mb-2 text-sm text-gray-400">ロール構成（変更可能）</label>
                    ...
                    <div class="flex flex-wrap gap-2 mb-2">
                        <template x-for="(t, i) in times" :key="i">
                            <div class="flex items-center gap-1 bg-gray-800 rounded px-2 py-1">
                                ...
                                <button type="button" @click="removeTime(i)" class="text-red-400 hover:text-red-300 text-lg leading-none">×</button>
                            </div>
                        </template>
                    </div>
        ...
        <div class="card p-5 mb-6">
            ...
            <div class="flex gap-4">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="attendance_mode" value="ternary" {{ old('attendance_mode','ternary') === 'ternary' ? 'checked':'' }} class="accent-amber-400">
                    <span class="text-green-400 font-bold">○</span>
                    <span class="text-yellow-400 font-bold">△</span>
                    <span class="text-red-400 font-bold">×</span>
                    <span class="text-sm text-gray-400">（3種類）</span>
                </label>
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="attendance_mode" value="binary" {{ old('attendance_mode') === 'binary' ? 'checked':'' }} class="accent-amber-400">
                    <span class="text-green-400 font-bold">○</span>
                    <span class="text-red-400 font-bold">×</span>
                    <span class="text-sm text-gray-400">（2種類）</span>
                </label>
            </div>
        </div>
